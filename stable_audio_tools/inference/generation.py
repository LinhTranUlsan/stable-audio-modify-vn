...
def generate_diffusion_cond(
        ...
        init_noise: tp.Optional[torch.Tensor] = None,
        init_sigma: tp.Optional[float] = None,
        partial_steps: tp.Optional[int] = None,
        sigma_min: float = 0.1,
        sigma_max: float = 500,
        sampler_type: str = "k-heun",
        **sampler_kwargs
        ) -> tp.Union[torch.Tensor, tp.Tuple[torch.Tensor, float]]: 
    """
    ...
    Args:
        ...
        init_noise: Initial noise tensor for continuing diffusion.
        init_sigma: Initial sigma value for continuing diffusion.
        partial_steps: Number of steps to run for partial diffusion.
        sigma_min: Minimum sigma value for the diffusion schedule.
        sigma_max: Maximum sigma value for the diffusion schedule.
        sampler_type: Type of sampler to use (e.g., "k-heun", "dpmpp-3m-sde").
        **sampler_kwargs: Additional keyword arguments to pass to the sampler.    
    """

    # Debug: Log input parameters
    print(f"[DEBUG] generate_diffusion_cond: return_latents={return_latents}, sampler_type={sampler_type}")
    ...
    if init_noise is not None:
        noise = init_noise
        sigma_max = init_sigma if init_sigma is not None else sigma_max
    else:
        noise = torch.randn([batch_size, model.io_channels, sample_size], device=device)
        sigma_max = sigma_max
    ...
    diff_objective = model.diffusion_objective
    effective_steps = partial_steps if partial_steps is not None else steps
    sampler_kwargs["sigma_max"] = sigma_max
    sampler_kwargs["sigma_min"] = sigma_min

    # Debug: Log filtered kwargs
    filtered_kwargs = {k: v for k, v in sampler_kwargs.items() if k not in ['return_latents', 'init_noise', 'init_sigma', 'partial_steps', 'sigma_min', 'sigma_max', 'sampler_type', 'return_latent']}
    print(f"[DEBUG] generate_diffusion_cond: filtered_kwargs={filtered_kwargs}")

    if diff_objective == "v":    
        # k-diffusion denoising process go!
        sampled = sample_k(model.model, noise, init_audio, effective_steps, sampler_type=sampler_type, **filtered_kwargs, **conditioning_inputs, **negative_conditioning_tensors, cfg_scale=cfg_scale, batch_cfg=True, rescale_cfg=True, device=device)
    elif diff_objective in ["rectified_flow", "rf_denoiser"]:
        sampled = sample_rf(model.model, noise, init_data=init_audio, steps=effective_steps, **filtered_kwargs, **conditioning_inputs, **negative_conditioning_tensors, dist_shift=model.dist_shift, cfg_scale=cfg_scale, batch_cfg=True, rescale_cfg=True, device=device)

    # Debug: Log sampled shape
    print(f"[DEBUG] generate_diffusion_cond: sampled shape={sampled.shape}")
    ...
    print(f"[DEBUG] generate_diffusion_cond: returning latents={return_latents}, sigma_max={sigma_max}")
    # Return latent and final sigma if requested
    if return_latents:
        return sampled, sigma_max
