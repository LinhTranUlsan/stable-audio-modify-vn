...
#     def save_prefix(self, prefix_latent, final_sigma, prompt, steps, cfg_scale, seed, filename=None):
#         if filename is None:
...
#         description='Branching Diffusion Audio Generation with Prefix Caching',
...
#         base_prompt = "Format: Band | Genre: Rock | Subgenre: Indie Rock | Instruments: echoing electric guitars with chorus, well recorded drum-kit, Electric Bass, occasional soaring harmonies | Moving, Epic, Climactic| BPM: 125"
        
#         branches = [
#             {
#                 "name": "verse_piano",
#                 "prompt": "Genre: Indie | Subgenre: Pop Rock, Indie Rock | Instruments: piano, drum machine, organ | Moods: soft, happy | Tempo: Medium",
#                 "output": "output_verse_piano.wav",
#                 "cfg_scale": 10.0
#             },
#             {
#                 "name": "verse_acoustic",
...
#                 "output": "output_verse_acoustic.wav",
#                 "cfg_scale": 18.0
#             },
#             {
...
#                 "output": "output_verse_electronic.wav",
#                 "cfg_scale": 22.0
#             }
#         ]
...
#         print(f"[{datetime.now().strftime('%H:%M:%S')}] main [INFO]: Generating {len(branches)} branches from prefix")
        
#         for i, branch in enumerate(branches, 1):
#             print(f"[{datetime.now().strftime('%H:%M:%S')}] main [INFO]: Branch {i}/{len(branches)}: {branch['name']}")
            
#             generate_branch_from_prefix(
#                 model, sample_rate, sample_size,
#                 prefix_latent, final_sigma,
#                 branch_prompt=branch['prompt'],
#                 output_path=branch['output'],
...
#         if not args.no_base:
#             print(f"[{datetime.now().strftime('%H:%M:%S')}] main [INFO]: - output_base.wav (base reference)")
#         for branch in branches:
#             print(f"[{datetime.now().strftime('%H:%M:%S')}] main [INFO]: - {branch['output']} ({branch['name']})")
#         print(f"[{datetime.now().strftime('%H:%M:%S')}] main [INFO]: Cached prefix: latent_cache/{args.cache_file}")
